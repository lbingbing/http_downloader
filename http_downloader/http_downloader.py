...
def get_remote_data(url, data=None, headers=None, try_num=60, verbose=False):
    ...
    all_headers = get_random_headers()
...
def download_url(url, file_path, verbose=False):
    if os.path.isfile(file_path):
        raise Exception("file '{}' already exists".format(file_path))
    else:
        return write_file(file_path, get_remote_data(url, verbose=verbose))
...
def download_urls(urls, dest_dir, file_names=None, file_name_pattern=None, worker_num=4, is_log=False):
    ...
    def worker(url, file_path):
        if os.path.isfile(file_path):
            ...
            bytes_num = download_url(url, file_path)
            delay()
        return bytes_num
...
if __name__ == '__main__':
    ...
    if args.mode == 'su':
        try:
            download_url(args.target_url, args.dest_file_path, verbose=args.verbose)
        except Exception as e:
            print('{0} [Error: {1}]'.format(e.url, e), file=sys.stderr)
    elif args.mode == 'fu':
        try:
            with open(args.urls_file_path) as f:
                urls = f.read().split()
            download_urls(urls, args.dest_dir_path)
        except Exception as e:
